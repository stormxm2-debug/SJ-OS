...
| 런처 | 하는 일 |
| --- | --- |
| **SJ OS 웹 배포** | 모바일 웹(PWA)을 빌드해 Cloudflare Pages 로 올림 |
| **SJ OS 앱 배포** | PC 데스크톱 설치본(.exe)을 만듦 |

새 PC를 세팅하거나 아이콘을 지웠을 때 다시 돌리면 됩니다.

> `.bat` 파일 자체를 바탕화면으로 **복사하면 안 됩니다.** 파일이 자기 위치에서 프로젝트
> 폴더를 찾는 구조라, 복사본은 폴더를 못 찾습니다. 반드시 이 스크립트로 **바로가기**를
> 만드세요.
