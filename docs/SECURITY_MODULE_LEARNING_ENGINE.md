...
## 10. Phase 2 상세 설계 — "여러 보험사 동시 실행 → PC 먹통" 해결

CEO 지정 최우선 타깃. 이 절은 2단계(자동 정리) 설계를 **먹통 시나리오에 맞춰 구체화**한다.
여전히 안전 최우선 — 잘못 끄면 백신·인증·시스템이 죽어 더 큰 사고이므로, 아래처럼 **매우 보수적**으로 간다.

### 10.1 먹통의 근본 원인(무엇을 없애야 하나)
1. **입력 후킹 체인 충돌** — 보험사마다 키보드 보안(TouchEn/nProtect/AntiKeylogger…) **키보드 필터 드라이버**를
   설치. 여러 개가 동시에 입력 스택에 끼면 키/마우스가 멈춘다(=먹통).
2. **같은 공용 모듈의 버전 충돌** — A사와 C사가 같은 보안 모듈을 **다른 버전**으로 올리면 상호 간섭.
3. **자원 고갈** — 전용 서비스·드라이버가 누적 상주 → CPU/핸들/메모리 고갈.

→ 핵심 해법은 **"한 번에 한 보험사의 보안 스택만 활성"**으로 만드는 것. 죽이기보다 **겹치지 않게** 하는 게 먼저.

### 10.2 현실적 제약 — 커널 드라이버는 "죽이기"가 안 된다
- **프로세스**: `CloseMainWindow` → 대기 → (허용 시) 강제. 가능.
- **서비스**: `Stop-Service`(STOPPABLE인 경우) — 종속성 고려. 가능하지만 보안 서비스는 보호가 걸려 거부될 수 있음.
- **커널/필터 드라이버**: 런타임 언로드가 대부분 **불가능/위험**(부팅·시스템 시작 유형). 억지 언로드는 BSOD 위험.
  → 드라이버는 **런타임에 끄지 않는다.** 대신 두 경로:
  - (a) **예방**: 다른 보험사 스택이 이미 떠 있으면 두 번째 스택을 **띄우지 않도록** 유도(순차 사용).
  - (b) **재부팅-정리**: 서비스 시작유형을 `수동/사용안함`으로 바꿔 **다음 부팅에 깨끗한 상태**로. 재부팅 필요.

즉 Phase 2의 실효 액션은 **① 프로세스 정상 종료 + ② 정지 가능한 서비스 정상 정지 + ③ 정지 불가 항목은 재부팅-정리 계획**의 조합이다.

### 10.3 트리거 & 모드
- **수동 모드 전환(우선 구현):** 사용자가 "지금 **C사만** 쓸게" 선택 → C사 스택만 남기고 **다른 보험사 전용**만 정리.
- **먹통 위험 예방(경고):** 자동 감지가 "이미 A사 스택 활성인데 C사 스택이 뜨려 함(§9 충돌 진단 high)"을 보면
  **정리 후 실행** 제안을 먼저 띄운다(자동으로 죽이지 않음 — 확인 우선).

### 10.4 정리 대상 선정 알고리즘 (`planModeSwitch(targetInsurerId)`)
```
active   = 대상 보험사(target)의 { insurer-exclusive + 필요한 insurer-shared } 요소
stopList = 다른 보험사의 insurer-exclusive 요소 중, 아래를 모두 만족:
  - 현재 실행 중
  - controlEligible === true (신뢰점수/반복/서명 충족)
  - protectedReasons 비어 있음 (§6)
  - category ∉ {windows-system, security-av, business-common, never-terminate, unknown}
  - 공용 아님(insurer-shared 제외)
sharedStop = insurer-shared 중 sharedRefCount(target 제외 실행중 보험사)==0 인 것만
```
각 대상의 실행 방식 판정:
- process → `graceful-stop`(CloseMainWindow, 8s) → 실패+허용 시 `force-stop`.
- 정지 가능 service → `graceful-stop`(Stop-Service, 종속성 먼저).
- 정지 불가 service/driver → **런타임 종료 안 함**. `reboot-clean`(StartMode=Manual) 항목으로 분류 + "재부팅 권장" 표시.

### 10.5 안전 게이트 (실행 직전 재확인 — 이중 확인)
승격 조건(§4.4)을 통과한 요소라도 **종료 직전에 다시** 검사:
1. §6 보호 규칙 재평가(그 사이 다른 보험사가 그 공용 모듈을 쓰기 시작했는지 등).
2. 미저장 Office 문서 실시간 점검 → 있으면 해당 프로세스 제외.
3. `pre-change` 스냅샷 저장(§7).
4. 화이트리스트 이외의 드라이버 종료 절대 금지(런타임 종료 목록에 driver 없음).

### 10.6 롤백 (먹통 특화)
- 정리 후 **검증 스냅샷**: 대상 보험사 전산이 정상인지 + 시스템 응답성(핵심 서비스 생존) 확인.
- 실패(전산 안 뜸/핵심 서비스 죽음) → `previousState`로 **재시작 복원**. 최대 2회, 초과 시 정지.
- `reboot-clean` 항목은 롤백도 "StartMode 원복"으로 처리(재부팅 전까지 실제 상태 불변이라 안전).

### 10.7 UX 흐름
```
[C사 모드] 클릭
  → (dry-run) "정리할 항목: A사 전용 서비스 2개 종료, 드라이버 1개는 재부팅 시 정리" 미리보기
  → 사용자 확인
  → 실행(정상 종료 우선) → 검증 → 결과 리포트
  → (재부팅-정리 있으면) "완전 정리하려면 재부팅하세요" 안내(강제 아님)
```
자동으로 몰래 죽이지 않는다. **미리보기 → 확인 → 실행 → 검증 → (필요시)재부팅 안내.**

### 10.8 Phase 2 자체의 점진 롤아웃
- **2a. Dry-run(시뮬레이션):** 무엇을 끌지 **보여만 준다.** 실제 종료 0. ← 여기부터 시작(안전).
- **2b. 정상 정지만(가역):** 프로세스 정상 종료 + 정지 가능 서비스만. 드라이버·강제종료 없음. 롤백 포함.
- **2c. 재부팅-정리 + (옵션) 강제:** StartMode 변경/재부팅 안내, 강제 종료는 제한시간·보호 재확인 통과 시에만.

### 10.9 성공 기준(검증 방법)
- 동일 PC에서 A·C사 전산을 순차 사용 시 **입력 먹통 재현 안 됨**.
- 정리 후 대상 보험사 전산 **정상 동작 + 시스템 응답성 유지**.
- 백신·인증·Windows 핵심 **한 번도 종료되지 않음**(로그로 증명).
- 실패 시 **자동 복원**되어 사용자 작업 손실 없음.

### 10.10 데이터 선행 조건
2b 이상 활성화 전에, 대상 보험사들이 각각 **`controlEligible`(3세션↑·신뢰 0.8↑·서명 유효)** 에 도달해야 한다.
→ 그래서 지금은 총무 PC에서 **자동 감지 ON으로 데이터부터 축적**하는 것이 선행 작업이다.
