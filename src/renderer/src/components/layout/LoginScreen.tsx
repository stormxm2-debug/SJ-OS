...
export default function LoginScreen(): JSX.Element {
  ...
  return (
    <div className="flex min-h-screen w-screen items-center justify-center overflow-y-auto bg-gradient-to-br from-[#eef3fb] via-[#f5f8fd] to-[#e9eff9] p-6">
      <div className="w-full max-w-sm rounded-2xl border border-slate-800 bg-white p-7 shadow-lg">
        ...
        <form onSubmit={onLogin} className="space-y-2">
          <div className="flex items-center gap-2 rounded-xl border border-slate-800 px-3">
        ...
        {setupPhone ? (
          <div className="mt-3 rounded-xl border border-[#c7d2fe] bg-indigo-50/50 p-3">
            ...
            {setupMsg ? <p className="mt-2 text-[11px] text-slate-300">{setupMsg}</p> : null}
          </div>
        ) : null}
        ...
        {forgotOpen ? (
          <div className="mt-2 rounded-xl border border-slate-800 bg-slate-950 p-3">
            <input value={forgotPhone} onChange={(e) => setForgotPhone(e.target.value)} inputMode="tel" placeholder="휴대폰 번호" className="w-full rounded-lg border border-slate-800 px-3 py-2 text-sm focus:outline-none" />
            ...
            {forgotMsg ? <p className="mt-2 text-[11px] text-slate-300">{forgotMsg}</p> : null}
          </div>
        ) : null}
        ...
        {!supabaseConfigured ? (
          <div className="mt-4 border-t border-slate-800 pt-3">
            <div className="rounded-lg border border-slate-800 bg-white p-2">
              <div className="mb-1 text-[10px] font-semibold text-slate-500">로컬 MVP 로그인 (개발/테스트)</div>
              <div className="flex flex-wrap gap-1">
                {DEMO_USERS.map((u) => (
                  <button key={u.id} type="button" onClick={() => login(u)} className="rounded border border-slate-800 bg-white px-2 py-1 text-[11px] text-slate-300 hover:bg-slate-950">
